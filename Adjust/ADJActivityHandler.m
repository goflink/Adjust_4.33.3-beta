...
preLaunchActions:(ADJSavedPreLaunch*)preLaunchActions
{
    ...
    Class deeplinkBroadcastClass = NSClassFromString(@"ADJDeeplinkBroadcast");
    if (deeplinkBroadcastClass == nil) {
        [ADJAdjustFactory.logger verbose:@"ADJDeeplinkBroadcast class not found in the app"];
    } else {
        SEL configureDeeplinkBroadcastSelector = NSSelectorFromString(@"configureDeeplinkBroadcast");
        if ([deeplinkBroadcastClass respondsToSelector:configureDeeplinkBroadcastSelector]) {
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Warc-performSelector-leaks"
            [deeplinkBroadcastClass performSelector:configureDeeplinkBroadcastSelector];
#pragma clang diagnostic pop
            [ADJAdjustFactory.logger verbose:@"Call to ADJDeeplinkBroadcast's configureDeeplinkBroadcast method made"];
        }
    }
...
- (void)appWillOpenUrlI:(ADJActivityHandler *)selfI
    ...
    Class deeplinkBroadcastClass = NSClassFromString(@"ADJDeeplinkBroadcast");
    if (deeplinkBroadcastClass == nil) {
        [ADJAdjustFactory.logger warn:@"ADJDeeplinkBroadcast class not found in the app"];
    } else {
        SEL processDeeplinkBroadcastSelector = NSSelectorFromString(@"processDeeplink:");
        if ([deeplinkBroadcastClass respondsToSelector:processDeeplinkBroadcastSelector]) {
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Warc-performSelector-leaks"
            [deeplinkBroadcastClass performSelector:processDeeplinkBroadcastSelector withObject:url];
#pragma clang diagnostic pop
            [ADJAdjustFactory.logger verbose:@"Call to ADJDeeplinkBroadcast's processDeeplink: method made"];
        }
    }
